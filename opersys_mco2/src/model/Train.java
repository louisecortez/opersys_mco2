...
public class Train implements Runnable, Pausable {
	...
	private Task<Integer> taskTrain;
	...
	public Train(int testId, int capacity, TrackManager tracker, Simulation simulation, APopup pops) {
		this.testId = testId;
		listPassenger = new ArrayList<Passenger>();
		this.capacity = capacity;
		this.simulation = simulation;
	...
	public Train(int testId, int capacity, TrackManager tracker, Simulation simulation, Controller c) {
		this.testId = testId;
		listPassenger = new ArrayList<Passenger>();
		this.capacity = capacity;
		this.simulation = simulation;
	...
	public void run() {
		...
		while(!simulation.allPassengersExited()) {
			...
			if(ttracker.moveTrain(this, stationCurr)) {
				...
				System.out.println("Animation thread start");
//				try {
//					t.join();
...
//				}
				Platform.runLater(new Runnable() {
					...
					@Override
					public void run() {
						pops.moveTrainGUI(testId);
					}
					
				});
				
				System.out.println("Animation thread finish");
